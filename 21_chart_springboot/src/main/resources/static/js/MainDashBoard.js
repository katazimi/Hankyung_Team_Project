...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadWatchlist();
    updateExchangeRate();
    setRankMode('rising'); // 처음은 급상승
    
    // 2. 웹소켓 연결 (장 운영 시간 체크)
    if (isMarketOpen()) {
        if (APPROVAL_KEY) connectWebSocket();
        else console.log("웹소켓 키가 존재하지 않습니다.");
    } else {
        console.log("🌙 현재는 장 마감 시간입니다. 웹소켓 미연결.");
    }
...
function loadRankByMode() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Top 30 (모달용 데이터 준비 - 열릴 때 렌더링하도록 저장해두거나 여기서 미리 렌더링)
            // 여기서는 미리 렌더링
...
function loadWatchlist() {
    fetch('/api/watchlist')
        ...
        .then(data => {
            ...
            else document.getElementById('watchlist').innerHTML = '<tr><td colspan="4" style="text-align:center; padding:20px; color:#999;">로그인 후 이용해주세요.</td></tr>';
        })
        .catch(err => console.error("관심종목 로딩 실패:", err));
}
...
function setRankMode(mode) {
    ...
    if (mode === 'rising') {
        label.textContent = '급상승';
        label.style.color = '#f55';
    } else {
        label.textContent = '급하락';
...
function openModal() {
    ...
    if (title) {
        title.textContent = currentRankMode === 'rising' ? '🔥 실시간 급상승 Top 30' : '📉 실시간 급하락 Top 30';
    }
...
function openManageModal() {
    fetch('/api/watchlist')
        ...
        .catch(err => console.error("관리 목록 로딩 실패:", err));
}
...
function renderManageList(data) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="2" style="text-align:center; padding:20px; color:#999;">등록된 관심종목이 없습니다.</td></tr>';
        return;
    }

    data.forEach(item => {
        const row = document.createElement('tr');
        // 관리 목록에서는 클릭 시 차트 이동 안 함 (삭제 버튼 오동작 방지)
        row.innerHTML = `
            ...
            <td class="text-center">
                <button class="btn-delete-sm" onclick="deleteItem('${item.code}', '${item.name}')">삭제</button>
            </td>
        `;
        tbody.appendChild(row);
    });
...
function deleteItem(code, name) {
    ...
    .then(res => {
        if (res.ok) {
            openManageModal(); // 목록 갱신
            loadWatchlist();   // 메인화면 갱신
        } else alert("삭제 실패");
    });
}
...
function loadInitialIndices() {
     fetch('/api/market/indices') 
        ...
        .catch(err => console.warn("초기 지수 로딩 실패:", err));
}
...
function updateExchangeRate() {
    fetch('/api/market/exchange-rate')
        ...
        .then(text => {
            ...
            if (el && rate) {
                el.innerText = parseFloat(rate).toLocaleString() + " 원";
                el.className = "index-value flat"; 
            }
        })
        .catch(err => console.error("환율 실패:", err));
}
...
function updateIndexUI(targetId, price, sign, change, rate) {
    ...
    if (sign === "1" || sign === "2" || parseFloat(change) > 0) { 
        colorClass = "up"; signIcon = "▲"; 
    } else if (sign === "4" || sign === "5" || parseFloat(change) < 0) { 
        colorClass = "down"; signIcon = "▼"; 
    }
